...
#we'll need to regularize all tracks, but interpolate locations at finer temporal res for ID 6470
...
#All tracks have sufficient data over at least 1 year
...
# Remove bursts w/ very few points (e.g., n < 5)
dat3 <- dat2 |> 
...
ssm_res <- grab(rw_fit, what = "predicted")  #if what = "fitted", returns locations at observed timestamps
...
ggplot() +
  geom_path(data = dat3, aes(lon, lat), color = "black", linewidth = 0.25) +
  geom_point(data = ssm_res, aes(lon, lat), color = "red", size = 0.1) +
  theme_bw() +
